...
- **Phase 6B — Production track (user-directed, added 2026-07-12).** Two steps, each reviewer-gated, run after the Phase 6 gate:
  ...
  2. web-frontend (Fable 5, high): customer-facing Next.js + TypeScript + Tailwind + shadcn/ui frontend consuming the API with Zod-validated types, MapLibre or deck.gl map. Portfolio-grade UI. Two user directives (2026-07-12): derive the frontend types from the FastAPI OpenAPI schema instead of hand-writing them, so the contract stays single-source; render the SHAP waterfall natively from the structured top_drivers (signed shap_eur values) and ignore the shap_plot_png_base64 field, which stays in the response for Streamlit.
